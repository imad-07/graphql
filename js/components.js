...
let donut = (percent, value) => document.createRange().createContextualFragment(
  `<svg id="donut" viewBox="0 0 40 40" width="100%" height="auto">
...
let bars = (maxWidth, barHeight, spacing, ratio) => document.createRange().createContextualFragment(
  `<svg id="bars" viewBox="0 0 ${maxWidth+50} ${barHeight * 2 + spacing * 3}" width="100%" height="auto">
      <rect x="10" y="${spacing - 5}" width="${maxWidth}" height="${barHeight}" stroke="black" stroke-width="2" fill="rgb(220, 180, 140)" />
      <text x="15" y="${spacing + barHeight - 20}" font-size="14" fill="white">Audit Given: ${ratio.toFixed(3)}</text>
      <rect x="10" y="${barHeight + spacing * 2}" width="${(1 / ratio) * maxWidth}" height="${barHeight}" stroke="black" stroke-width="2" fill="#3c2c1d" />
      <text x="15" y="${barHeight * 2 + spacing * 2 - 15}" font-size="14" fill="white">Audit Received: 1.000</text>
  </svg>`
);
const query = `{
    user {
